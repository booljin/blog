...
笔者作为10多年的游戏服务器主程，积累了一些自己的方法论。[第四篇](./gaia-04.md) 中的 send/call，就是笔者针对隔离手段复杂的解决方法之一。一个 call 函数调用，就像本地函数调用一样自然，开发人员不需要理解背后的复杂逻辑，就能专注于实现自己的业务。但这肯定不够，更多设计会在后续的 API 模块中持续展开。
